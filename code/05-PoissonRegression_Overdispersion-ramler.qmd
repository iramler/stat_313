...
cdat <- cdat %>%
              mutate(region, 
                     region = fct_recode(region, "S" = "SW", "S"="SE"))
...
```{r}
mod1 = glm(nv ~ type + region + region:type, family = "poisson", 
           data=cdat, 
           offset = log(enroll1000) # be sure to take the log(covariate) here!
           )
summary(mod1)
```

Notes: Baseline groups - College (Type) and Central (Region)

CHECK LACK-OF-FIT:

```{r}
ts_lof <- summary(mod1)$deviance # lack of fit test statistic
df_lof <- summary(mod1)$df.residual # df of lack of fit test
pval_lof <- pchisq(ts_lof, df_lof, lower.tail = FALSE)
pval_lof
```

We have a lot of type/region combos where var $\neq$ mean.
...
Same structure, but standard errors on individual coefs are larger now.

$SE_Q(\hat{\beta_i}) = \sqrt{\hat{\phi}} \cdot SE(\hat{\beta_i})$ with
...
mod3 = MASS::glm.nb(nv ~ type + region + region:type, data=cdat2, weights = offset(log(enroll1000)),  link = log)
summary(mod3)
```
